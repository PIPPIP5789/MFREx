...
/**
 * Represents a JEI "recipe" for the Masonry bench.
 */
public class JEIMasonryBenchRecipe implements IRecipeWrapper {

    private final ItemStack result;
    private final MasonryBenchRecipeBase recipe;

    private final List<List<ItemStack>> ingredients;

    public JEIMasonryBenchRecipe(MasonryBenchRecipeBase recipe, IStackHelper stackHelper) {
        ...
        List<List<ItemStack>> ingredients = stackHelper.expandRecipeItemStackInputs(RecipeHelper.expandPattern(
                ...
                MasonryBenchRecipeBase.MAX_WIDTH, MasonryBenchRecipeBase.MAX_HEIGHT));
        this.recipe = recipe;
        this.result = recipe.getMasonryBenchRecipeOutput();
    ...
    public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        ...
        GuiHelper.renderToolIcon(minecraft.currentScreen, "Masonry", recipe.getMasonryBenchTier(), recipeWidth - 23, recipeHeight - 48, true, true);
        ...
        if (GuiHelper.isPointInRegion(recipeWidth - 23, recipeHeight - 98, 20, 20, mouseX, mouseY, 0, 0)) {
            ...
            // Shows the Masonry tooltip text with the minimum Masonry tier
            String s2 = I18n.format("tooltype.Masonry") + ", " + (recipe.getMasonryBenchTier() > -1
                    ? I18n.format("attribute.mfcrafttier.name") + " " + recipe.getMasonryBenchTier()
                    : I18n.format("attribute.nomfcrafttier.name"));
            minecraft.fontRenderer.drawStringWithShadow(s2, (float) ((recipeWidth / 2) - minecraft.fontRenderer.getStringWidth(s2) / 2), (float) 84, 16777215);
        } else {
