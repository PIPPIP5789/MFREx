...
/**
 * Represents a JEI "recipe" for the Tailor bench.
 */
public class JEITailorBenchRecipe implements IRecipeWrapper {

    private final ItemStack result;
    private final TailorBenchRecipeBase recipe;

    private final List<List<ItemStack>> ingredients;

    public JEITailorBenchRecipe(TailorBenchRecipeBase recipe, IStackHelper stackHelper) {
        ...
        List<List<ItemStack>> ingredients = stackHelper.expandRecipeItemStackInputs(RecipeHelper.expandPattern(
                ...
                TailorBenchRecipeBase.MAX_WIDTH, TailorBenchRecipeBase.MAX_HEIGHT));
        this.recipe = recipe;
        this.result = recipe.getTailorBenchRecipeOutput();
    ...
    public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        ...
        GuiHelper.renderToolIcon(minecraft.currentScreen, "Tailor", recipe.getTailorBenchTier(), recipeWidth - 23, recipeHeight - 48, true, true);
        ...
        if (GuiHelper.isPointInRegion(recipeWidth - 23, recipeHeight - 98, 20, 20, mouseX, mouseY, 0, 0)) {
            ...
            // Shows the Tailor tooltip text with the minimum Tailor tier
            String s2 = I18n.format("tooltype.tailor") + ", " + (recipe.getTailorBenchTier() > -1
                    ? I18n.format("attribute.mfcrafttier.name") + " " + recipe.getTailorBenchTier()
                    : I18n.format("attribute.nomfcrafttier.name"));
            minecraft.fontRenderer.drawStringWithShadow(s2, (float) ((recipeWidth / 2) - minecraft.fontRenderer.getStringWidth(s2) / 2), (float) 84, 16777215);
        } else {
